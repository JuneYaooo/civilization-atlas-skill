...
[播放／下载 WorkBuddy 结果浏览视频](../../assets/workbuddy/results-browsing.mp4)

约 15 秒，1200 × 800，无音频。连续调用窗口截图接口采集 117 帧，按实际时间戳编码为 MP4；平均约 8 帧／秒，滚动操作期间最大采样间隔约 1 秒。画面展示本次已完成回答的指标与失败案例，没有模拟输出或重新生成回答。它是结果浏览记录，不是从输入问题到生成完成的全程录像。
